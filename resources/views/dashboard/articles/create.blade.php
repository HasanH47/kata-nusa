...
<div class="mx-auto">
    <div class="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <form id="article-form" action="{{ route('dashboard.articles.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <textarea id="summary"
                          ...
                          required
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500"
                          placeholder="Tuliskan ringkasan singkat artikel (maksimal 150 karakter)">{{ old('summary') }}</textarea>
                @error('summary')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="hidden" name="body" value="{{ old('body') }}" id="body" required>
                @error('body')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="hidden" name="tags[]" id="tags" value="{{ old('tags') ? implode(',', old('tags')) : '' }}">
                @error('tags')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
